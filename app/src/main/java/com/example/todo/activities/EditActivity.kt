...
class EditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val taskText = intent.getStringExtra("TASK_TEXT")
            ?: throw RuntimeException("task's text cannot be null!")
        val tasksLocal: Int = intent.getIntExtra("TASK_ID", -1)
        val tasksLatitude: Double = intent.getDoubleExtra("TASK_LAT", 0.0)
        val tasksLongitude: Double = intent.getDoubleExtra("TASK_LON", 0.0)
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                dao.updateTask(
                    Tasks(
                        ...
                        binding.coordinatesLatTextBox.text.toString().toDouble(),
                        binding.coordinatesLongTextBox.text.toString().toDouble(),
                        taskStatus
                    )
                )
